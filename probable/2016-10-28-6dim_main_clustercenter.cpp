...
int main(int argc, char** argv) {
    if (argc <= 1) {
        cerr << "\nUSAGE:\n\n"
        << "./6dimprob [-i inputfile] [-c clustercenterfile]\n\n"
        << "where\n\n"
    ...
    clock_t t;
    t = clock();
    int i = 0; string infile; string cenfile;
    while (i<argc) {
        if (!strcmp(argv[i], "-i")) {
            infile = argv[++i];
        }
    ...
    cout << infile.substr(8,6) << endl;
    int filenum = atoi(infile.substr(8,6).c_str());   
    cout << filenum << endl; 
    ...
    vector<double> cencoords; //contains all clustercenter coordinates

    ifstream cenput(cenfile.c_str());
    getline(cenput, strtemp); //skip header
    while (!cenput.eof()) {
	getline(cenput, strtemp);
	if (!strtemp.empty()) {
		temp = atof(strtemp.substr(30, 8).c_str());
		cencoords.push_back(temp);
		temp = atof(strtemp.substr(38, 8).c_str());
		cencoords.push_back(temp);
		temp = atof(strtemp.substr(46, 8).c_str());
		cencoords.push_back(temp);
	}
    }

	//All center coordinates stored in cencoords, no organization. Water 1 coordinates at 0,1,2.
	...
	*/ 
	...
	//code that translates the winning position back to the clustercenter position

	int cenpos = (filenum-1)*3;
	//cenpos describes which clustercenterfile coordinate we want
	

	double disx = winner[0]-cencoords[cenpos];
	double disy = winner[1]-cencoords[cenpos+1];
	double disz = winner[2]-cencoords[cenpos+2];
	//disx,y,z describes the translation to occur on the most probable configuration to centralize it within the hydration site
